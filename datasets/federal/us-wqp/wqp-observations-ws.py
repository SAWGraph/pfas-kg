...
def main():
    ...
    kg = triplify(df)
    kg.serialize(output_dir /'wqp_observations.ttl', format='turtle')

def load_data():
    df = pd.read_csv(data_dir / 'ME-pfas-results.csv')
...
def get_controlledvocabs():
    ...
    metadata_files = ['Characteristic'] #'Activity Media', 'Sample Collection Method', 'Activity Media Subdivision', 'Analytical Method',  "Organization", 'Monitoring Location Type'
    cv = {}
    #define which column is the primary key identifier for each metadata table. 
    identifier = {'Activity Media': 'index', 
                  'Characteristic': 'Name', 
                  'Sample Collection Method': 'ID'}
    for filename in metadata_files:
        #print(filename)
        data_df = pd.read_csv(metadata_dir / f"{filename}.csv", header=0, encoding='ISO-8859-1', index_col='Name')
...
def get_attributes(row):
    ...
    sample = {
        ...
        'project_id': row['Project_Identifier'], # annotation for sampling project (could be sampling collection)
        'org_id': row['Org_Identifier'], #organization that does sampling
    ...
    if pd.notnull(row['Project_Name']):
        sample['project_name'] = row['Project_Name']
    ...
    if pd.notnull(row['SampleCollectionMethod_Identifier']): #sampling method info in a nested dictionary
        sample['collectionMethod'] = {'id': row['SampleCollectionMethod_Identifier'], 
                                      'name': row['SampleCollectionMethod_Name']}
    ...
    if pd.notnull(row['SampleCollectionMethod_EquipmentName']):
        result['equipment'] = row['SampleCollectionMethod_EquipmentName']
    if pd.notnull(row['Result_ResultDetectionCondition']) and row['Result_ResultDetectionCondition'] == 'Not Detected':
        result['non-detect'] = True
    if pd.notnull(row['ResultAnalyticalMethod_Identifier']):
        result['analytical_method'] = row['ResultAnalyticalMethod_Identifier']
...
def get_iris(sample, samplepoint, result):
    ...
    if 'taxon' in sample.keys():
        taxon_short = sample['taxon'][0] + "."+ ''.join(word for word in re.findall(r"\b[a-z]\w*",sample['taxon']))#'_'.join([word[0:3] for word in str(sample['taxon']).split()])
        iris['taxon'] = prefixes['us_wqp_data'][f"d.wqp.biologicalTaxon.{taxon_short}"]
    else:
        iris['taxon'] = ''
    ...
    iris['quantityValue'] = prefixes['us_wqp_data'][f"d.wqp.quantityValue.{result['id']}"]
...
def triplify(df):
    ...
    for idx, row in df.iterrows():
        ...
        #triplify sample
        kg.add((iris['sample'], RDF.type, prefixes["us_wqp"]["WQP-Activity"]) )
        ...
        observation_po = { #list of optional triples for each observation (based on data)
            ...
            'analysis_date': (prefixes['coso']['analysisDate'], Literal(str(result['analysis_date']), datatype=XSD.date)) if 'analysis_date' in result.keys() else ''
        }
        ...
        kg.add((iris['measurement'], prefixes['coso']['observedProperty'],prefixes['coso']['SingleContaminantConcentrationQuantity'] ))
