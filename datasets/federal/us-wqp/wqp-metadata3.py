...
metadata_files = [ 'Activity Media', 'Analytical Method', 'Characteristic', 'Sample Collection Method', "Organization", 'Monitoring Location Type', 'Taxon', 'Quantitation Limit Type'] #'Activity Media Subdivision',
...
def triplify_media(df, _PREFIX):
    ...
    for idx, row in df.iterrows():
        ...
        kg.add((media_iri, RDF.type, _PREFIX["us_wqp"]["ActivityMedia"]) )
...
def triplify_analytical_method(df, _PREFIX):
    ...
    for idx, row in df.iterrows():
        if ("retired" in row['ID']):
            ...
            ## method IRI
            method_iri = _PREFIX["us_wqp_data"][f"{'wqp.analyticalMethod'}.{method_unique_id}"]
            ...
            if not method_context:
                kg.add( (method_iri, _PREFIX["us_wqp_data"]['contextCode'], Literal(method_context, datatype = XSD.string)) )
            if not method_url:
                kg.add( (method_iri, _PREFIX["us_wqp_data"]['url'], Literal(method_url, datatype = XSD.anyURI)) )
...
def triplify_characteristic(df, _PREFIX):
    ...
    for idx, row in df.iterrows():
        ...
        if row['Group Name'] in ['PFAS,Perfluorinated Alkyl Substance', 'PFOA, Perfluorooctanoic Acid','PFOS, Perfluorooctane Sulfonate', 'Stable Isotopes', "Organics, PFAS"]:
            ...
            if parameter_cas_no:
                kg.add( (parameter_iri, _PREFIX["us_wqp"]['casNumber'], Literal(parameter_cas_no, datatype = XSD.string)) )
            if parameter_srd_id:
...
def triplify_organization(df, _PREFIX):
    ...
    for idx, row in df.iterrows():
        ...
        organization_ID_formatted = ''.join(e for e in str(organization_ID) if e.isalnum())
        ## organization IRI
        organization_iri = _PREFIX["us_wqp_data"][f"{'organizaton'}.{organization_ID_formatted}"]
                    
        ## specify organization instance and it's data properties
        kg.add( (organization_iri, RDF.type, _PREFIX["prov"]["Organization"]) )
        kg.add( (organization_iri, RDFS['label'], Literal(str(organization_ID))) )
        kg.add( (organization_iri, _PREFIX["us_wqp_data"]['organizationName'], Literal(organization_name, datatype = XSD.string)) )
        kg.add( (organization_iri, _PREFIX["us_wqp_data"]['organizationDescription'], Literal(organization_description, datatype = XSD.anyURI)) )
