...
def triplify(df):
    ...
    for idx, row in df.iterrows():
        ...
        kg.add((iris['PWS'], RDF.type, prefixes['us_sdwis'][str('PublicWaterSystem' + '-' + pws['Type'])]))  # should this be a class?
        if 'GWSW' in pws.keys():
            kg.add((iris['PWS'], RDF.type, prefixes['us_sdwis'][str('PublicWaterSystem' + '-' + pws['GWSW'])]))
        if 'Name' in pws.keys():
        ...
        if 'Source' in pws.keys():
            kg.add((iris['PWS'], prefixes['us_sdwis']['primarySource'], Literal(pws['Source'], datatype=XSD.string)))
